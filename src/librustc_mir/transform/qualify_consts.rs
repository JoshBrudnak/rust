        ...
        match *dest {
            ...
            Place::Projection(box Projection {
                ...
                && self.local_qualif[index]
                    .map_or(false, |qualif| qualif.contains(Qualif::NOT_CONST)) =>
            {
...
impl<'a, 'tcx> Visitor<'tcx> for Qualifier<'a, 'tcx, 'tcx> {
                            ...
                            if let Some(def) = base_ty.ty_adt_def() {
                                if def.is_union() {
                                    match this.mode {
                                        ...
                                        | Mode::Static | Mode::StaticMut | Mode::Const => {}
                                    }
                                }
                            }
            ...
            Operand::Constant(ref constant) => {
                if let ConstValue::Unevaluated(def_id, _) = constant.literal.val {
                    ...
                    if self.tcx.trait_of_item(def_id).is_some() {
                        ...
                        self.qualif
                            .restrict(constant.literal.ty, self.tcx, self.param_env);
                    }
                }
            }
        }
    ...
    fn visit_rvalue(&mut self, rvalue: &Rvalue<'tcx>, location: Location) {
        ...
        if let Rvalue::Ref(region, kind, ref place) = *rvalue {
            ...
            if is_reborrow {
                self.super_place(place, PlaceContext::Borrow { region, kind }, location);
            } else {
                ...
                match self.tcx.fn_sig(def_id).abi() {
                    Abi::RustIntrinsic | Abi::PlatformIntrinsic => {
                        ...
                        match &self.tcx.item_name(def_id).as_str()[..] {
                            | "size_of" | "min_align_of" | "needs_drop" | "type_id" | "bswap"
                            | "ctpop" | "cttz" | "cttz_nonzero" | "ctlz" | "ctlz_nonzero" => {
